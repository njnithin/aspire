<template>
  <nav class="app-menu">
    <div class="app-menu__header">
      <div class="app-menu__header-logo">
        <img
          class="app-menu__header-logo--large"
          src="@/components/AppMenu/img/aspire-logo.svg"
          alt="Aspire-Logo-Large"
        />
        <div class="app-menu__header-logo--small">
          <svg
    ...
    <div class="app-menu__strip">
      <template v-for="(eachLink, index) in menuData.links">
        <router-link class="app-menu__link" :to="eachLink.link" :key="index">
          <div class="app-menu__link-icon">
            <div
              :class="
                'app-menu__link-icon icon-' + eachLink.label.toLowerCase()
              "
              v-html="eachLink.svg"
            ></div>
          </div>
...
.app-menu {
  ...
  &__strip {
    ...
    min-height: 56px;
    background: $white-c;
    box-shadow: 0px -3px 6px $shadow-c-1;
    max-width: 100%;
    overflow-x: auto;
  }
  &__link {
